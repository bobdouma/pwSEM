...
2. Given a d-separation claim: X1 _||_X2|{C}, where C is the set of causal parents of either X1 or X2,  the generalized covariance statistic consists of conducting two regressions (X1~C and X2~C),  getting the residuals from these two regressions, and then inputting these two vectors of residuals to calculate the generalized covariance statistic.  The residuals used are default (i.e. "deviance") residuals since these have zero expected values and approximately constant variance, are exactly the residuals for an ordinary normal linear model, and are closest to the residuals of ordinary linear models for other distributional families.  They are the recommended type by Simon Wood, who developed the mgcv package and also developed much of the underlying theory (Wood 2017, Generalized Additive Models). These regressions can be of any type that are appropriate given the nature of the dependent variables (X1, X2) and the assumed functional form linking them to the causal parents.  The pwSEM() function uses the "deviance" residuals that are returned from either the gam() or gamm4() functions of the mgcv package.
...
5. Optionally, the dependency between any assumed dependent errors is calculated.  This is either a covariance and Pearson correlation for normally distributed variables, or a Spearman correlation otherwise; these are based on the "response" residuals' i.e. based on the actual (observed - predicted) values of the original variables, not the transformed predicted values based on the link function.
...
pwSEM(sem.functions,dependent.errors = NULL,data,use.permutations = FALSE,n.perms = 5000,do.smooth = FALSE,all.grouping.vars = NULL)
...
*dependent.errors*:	A list giving any dependent errors (correlated error variables).  Each element of this list is a pair of variables whose errors are hypothesized to be dependent (i.e. they have some unknown common cause) separated by two tildes (~). 
...
### Example 2: dependent endogenous errors, Poisson distributed variables and no nesting structure in the data
...
An important reference to the mgcv package, and the underlying theory, is:
...
The first one, generalized.covariance(), produces asymptotic null probabilities based on a standard normal distribution, and is appropriate for larger sample sizes.  How large?  Simulations suggest that you need at least 100 observations, but these simulations are not exhaustive.
